...
export default {
  Query: {
    ...
    getProductByCategory: async (_, { category, limit, page, filter }) => {
      try {
        let query = {};
        if (category) {
    ...
    getKeywords: async (parent, args, { req }) => {
      const keys = req.session.keywords;
      if (keys == undefined) {
        return '';
      } else {
        return keys;
  ...
  Mutation: {
    generateKeyword: async (_, { keyword }, { req }) => {
      try {
        let keys = req.session.keywords;
        if (keys !== undefined) {
          req.session.keywords = keys;
        } else {
          req.session.keywords = keyword;
        }
        return true
      } catch(e) {
        if (e) {
    ...
    incViews: async (_, { id }, { req, user }) => {
      try {
        const views = req.session.views;
        if (views == undefined) {
          let arr = [];
          arr.push(id);
          req.session.views = arr;
        } else {
