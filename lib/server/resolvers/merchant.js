...
export default {
  ...
  Mutation: {
    ...
    saveSettings: async (parent, { storeName, delivery, deliveryFee, methods }) => {
      console.log(JSON.parse(methods))
      try {
        let updated = await Merchant.findOneAndUpdate({ username: storeName }, {
          delivery,
          deliveryFee,
    ...
    registerMerchant: async (parent, args) => {
      try {
        ...
        if (newMerchant) {
          ...
          const downloadPath = path.join(__dirname, `../uploads/${username}/logo/`, 'logo');
