...
export default {
  ...
  Mutation: {
    ...
    updateStock: async (_, { id, count, option }) => {
      try {
        const currentShip = await Ship.findOne({ _id: id });
        ...
        if (currentShip) {
          ...
          if (option) {
            let res = await Product.updateOne({ _id: id, 'options.name': option }, {
              $inc: { 'options.$.quantity': (parseInt(count) * -1 ) }
            });
            if (res) {
              return true
            } else {
            ...
            let res = await Product.findOneAndUpdate({ _id: id }, { $inc: { quantity: (parseInt(count) * -1) } }, { new: true });
            if (res) {
              return true
            } else {
              return false
    ...
    ship: async (_, { customer, purchases, grandTotal, grandDelivery }, { client }, info) => {
      try {
        const currentUser = await User.findOne({ email: customer });
