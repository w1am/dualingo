...
const Purchase = ({
  ...
  spaceError, hocLoading, buyLoading, cartSuccess, resetCart, setError
}) => {
  const onClickEvent = async (buy) => {
    if (product.addOptions.length >= 1 && addOption == 'null') {
      ...
      if (product.options.length >= 1 || product.addOptions.length >= 1) {
        try {
          await addToCart(JSON.stringify(selected), selectedQuantity, addOption, selectedQuantity, null, buy);
          if (buy) {
        ...
        try {
          await addToCart(null, null, addOption, selectedQuantity, null, buy);
          if (buy) {
            location.assign('/cart')
          }
        } catch(e) {
    ...
    <Fragment>
      {
        (spaceError) && (
          <div style={{ maxWidth: '300px' }}>
            <Notification>This product is currently out of stock.</Notification>
            <br />
          </div>
        )
      }
      ...
      <Layout>
        <div>
          ...
          <CurrProduct.Purchase.Cart disabled={(parseInt(currentStock) <= 0) || spaceError} onClick={() => onClickEvent(false)}>
            {
               (hocLoading) ? (
                 <ButtonLoader color="primary" placeholder={null} loading={hocLoading} />
              ) : (
                <Fragment>
        ...
        <CurrProduct.Purchase.Buy
          disabled={(parseInt(currentStock) <= 0) || spaceError}
          style={{ marginLeft: 10 }}
          onClick={() => onClickEvent(true)}
        >
          {
            (buyLoading) ? (
              <ButtonLoader color="primary" placeholder={null} loading={buyLoading} />
            ) : (
              <Fragment>
                <Common.Icons.Default
                  style={{ color: 'white', paddingLeft: 0 }}
