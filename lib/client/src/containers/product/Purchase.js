...
const Purchase = ({
  ...
  spaceError, hocLoading, cartSuccess, resetCart, setError, coupon, couponError
}) => {
    ...
    if (product.addOptions.length >= 1 && addOption == 'null') {
      ...
      if (product.options.length >= 1 || product.addOptions.length >= 1) {
        if (coupon && coupon.length >= 1) {
          addToCart(JSON.stringify(selected), selectedQuantity, addOption, selectedQuantity, coupon);
        } else {
          addToCart(JSON.stringify(selected), selectedQuantity, addOption, selectedQuantity, null);
        }
      } else {
        if (coupon && coupon.length >= 1) {
          addToCart(null, null, addOption, selectedQuantity, coupon);
        } else {
  ...
  return (
    <Layout>
      <Message link={null} isOpen={couponError} message="Invalid Coupon" linkText={null} />

      <div>
        <Message link='/cart' reset={resetCart} isOpen={cartSuccess} message="Item added to cart" linkText='View cart' />
        <CurrProduct.Purchase.Cart onClick={() => onClickEvent(false)}>
          {
            hocLoading ? (
              <ReactLoading
                type="spin"
      ...
      <CurrProduct.Purchase.Buy style={{ marginLeft: 10 }} onClick={() => onClickEvent(true)}>
        {
          hocLoading ? (
            <ReactLoading
              type="spin"
...
graphql(
  ...
  Tags.Coupons.Mutations.verifyCoupon, { name: 'verifyCoupon' }
)(graphql(
