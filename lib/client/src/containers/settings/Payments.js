    ...
    <Pays.Container>
      {
        payments.map((payment, index) => (
          <Pays.Layout
            key={index}
            checked={((selectedPayments[payment.key] !== undefined) && selectedPayments[payment.key].status) ? true : false}
            key={payment.key}
          >
            <div
              onClick={() => updateSelected(payment.key, null, (selectedPayments[payment.key] == undefined) ? true : false)}
            >
