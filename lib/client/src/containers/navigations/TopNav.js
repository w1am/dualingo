...
const TopNav = ({ isOpen, setIsOpen, activate, setActivate, toggleSearch, setToggleSearch , count, history, loadCount, loadSubTotal, loadItems }) => {
  ...
  return (
    <Navigation.TopNav.Container>
      ...
      <Menu.Responsive.Layout>
        <Icon
          onClick={() => (setIsOpen(!isOpen), setActivate(!activate))}
          loader={<div><Skeleton height={20} width={20} /></div>}
        ...
        <div onClick={() => history.push('/cart')} style={{ margin: 'auto 0', position: 'relative', marginLeft: 20 }}>
          <Icon
            ...
            loader={<div style={{ marginLeft: 0, marginRight: 20 }}><Skeleton height={20} width={20} /></div>}
            src={Cart}
          />
          { count >= 1 && <Trackers.Count>{count}</Trackers.Count> }
        </div>
        ...
        <Icon
          onClick={() => setToggleSearch(!toggleSearch)} src={Search}
          loader={<div><Skeleton height={20} width={20} /></div>}
        />
