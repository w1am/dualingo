...
const Inventory = ({
  ...
  const checkPurchaseLimit = (value, state, target) => {
    ...
    if (isNumber(newVal)) {
      ...
      } else if (newVal.substring(0, 1) == '0') {
        state('purchaseLimit', '', parseInt(newVal.substring(1, newVal.length).toString()));
      } else if (parseInt(newVal) > parseInt(formData['quantity'].value)) {
        state('purchaseLimit', '', formData['quantity'].value.toString())
      } else {
        state('purchaseLimit', '', parseInt(value))
      }
    } else {
      state('purchaseLimit', '', parseInt(formData['quantity'].value.toString()));
    }
  }
