          ...
          ((isAuthenticated().stores) && (isAuthenticated().stores.length > 1)) && (
            <div style={{ position: 'relative' }}>
              ...
              <Common.Containers.Dropdown ref={outsideClick} className={openModal ? 'fadeIn' : 'fadeOut'}>
                {
                  isAuthenticated().stores.map((store, i) => {
                    if (store.username !== params.name) {
                      return (
                        <Wrapper key={i} onClick={() => (location.assign(`/store/${store.username}/orders`), setOpenModal(false))}>
          ...
          <Table.Layout>
            <tbody>
              ...
              </Table.Wrapper>
              {
                ...
                ) : data.getStoreShips.purchases.map((ship, i) => (
                  <StoreOrderList key={i} ship={ship} />
                ))
              }
            </tbody>
          </Table.Layout>
