...
const Product = ({ history, location: { search }, match: { params: { id, storeName } } }) => {
  ...
  const [ generateKeyword ] = useMutation(Mutations.generateKeyword);
  ...
  useEffect(() => {
    if (!loading) {
      if (data.getProduct == null || data.getProduct == 'null') {
        ...
        generateKeyword({
          variables: { keyword: data.getProduct.categories[Math.floor(Math.random() * data.getProduct.categories.length)] }
        })
      }
    };
          ...
          (loading) ? (
            <Prod.CurrProduct.Container>
              ...
              <Prod.CurrProduct.Information.Container>
                <Information.Title><Skeleton height={30} /></Information.Title>
                <div style={{ margin: '10px 0px' }}>
                  {
                    [...Array(3)].map((cat, i) => (
                      <div key={i} style={{ marginRight: '10px', display: 'inline-block' }}>
                ...
                <Option.Wrapper>
                  {
                    [...Array(3)].map((x, i) => (
                      <Option.Layout
                        ...
                        selected="y"
                        currOption="x"
                        style={{ cursor: 'default', boxShadow: 'inset 0px 0px 0px 0.5px rgba(51, 51, 51, 0.2)' }}
                      >
