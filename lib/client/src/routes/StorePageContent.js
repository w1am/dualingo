...
const StorePageContent = ({ props: { match: { params: { tab, name }} }, setActiveTab, data, loading }) => {
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      {
        (tab == 'settings' && isAuthenticated().stores[0].username == name) ? (
        ...
        ) : (tab == 'pages' && isAuthenticated().stores[0].username == name) ? (
          <Stores.Content.Wrappers.Page>
            <Pages />
          </Stores.Content.Wrappers.Page>
        ) : (tab == 'products' && isAuthenticated().stores[0].username == name) ? (
          <Stores.Content.Wrappers.Page>
            <Common.Presentation>
              <Common.Headers.Page>Products</Common.Headers.Page>
              <Common.Elements.Divider />
              <StoreProducts />
            </Common.Presentation>
          </Stores.Content.Wrappers.Page>
        ) : (tab == 'orders' && isAuthenticated().stores[0].username == name) ? (
          <Stores.Content.Wrappers.Page>
            <StoreOrders />
          </Stores.Content.Wrappers.Page>
        ) : (tab == 'features' && isAuthenticated().stores[0].username == name) ? (
          <Stores.Content.Wrappers.Page>
            <StoreFeatures />
          </Stores.Content.Wrappers.Page>
        ) : (
