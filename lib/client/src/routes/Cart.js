...
const Cart = ({ items, inc, dec, resetCount, loadItems, subTotal, history, count }) => {
  ...
  const onSubmit = async () => {
    ...
    if (isAuthenticated().ok) {
      if ((items.length >= 1) && (objLen(types) <= 0)) {
        ...
        const { ok, ship: shipId } = res.data.ship;
        if (ok) {
        ...
        <Animations.FadeIn>
          ...
          <Common.Elements.Divider />
          {
            (items.length <= 0)
              ? (
                <div style={{ display: 'flex' }}>
                  <span>Looks like your cart is empty. Click </span>
          ...
          <FormNavigator placeholder="Next" loading={loading} hasErrors={error} onSubmit={onSubmit} />
        </Animations.FadeIn>
      </Common.Presentation>
    </div>
  )
