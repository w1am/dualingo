...
const UploadPage = ({
  ...
  const onSubmit = async () => {
    ...
    if (submitError || formError || checkAssets() || inventoryError() || checkOptionsError()) {
      ...
      if (customOptions.length >= 1) {
        ...
        // for (let x=0; x<=customOptions.length-1; x++) {
        //   addUserOptions.push({
        //     name: customOption[x].name,
        //     price: parseFloat(customOption[x].price),
        //   });
      ...
      if (uploads.length >= 1) {
        for (let x=0; x<=uploads.length-1; x++) {
          ...
          if (typeof optionFileUrl[x] == 'object') {
            ...
            path = uploadFileRes.data.uploadFile;
          } else {
  ...
  return (
    <div style={{ marginBottom: '8em' }}>
      ...
      <FormNavigator onSubmit={onSubmit} loading={loading} hasErrors={hasErrors} placeholder="Next" />
