...
const Shipping = ({ items, onInputChange, onFormSubmit, clearInputField, hasErrors, formData, loading, setLoading, subTotal, location: { search } }) => {
  ...
  const onSubmit = async () => {
    if (checked) {
          ...
          items.map(async item => {
            ...
            userItems.map(async i => {
              if ((i.option !== null) || (i.option !== 'null')) {
                res = await updateStock({ variables: { id: i.itemId, count: i.count, option: i.option } })
                if (res.data.updateStock) {
                ...
                res = await updateStock({ variables: { id: i.itemId, count: i.count, option: null } })
                if (res.data.updateStock) {
                  const res = await setShipInfo({
                    variables: {
                      id: values.session,
      ...
      setError('Review the terms before proceeding');
      setTimeout(() => {
        setError('');
      }, 2000)
    }
  ...
  return (
    <Common.Wrappers.Page style={{ minHeight: '100vh' }}>
      ...
      <Common.Form.Check.Wrapper>
        <Common.Form.Check.Input id="form_check" value={checked} onChange={() => setChecked(!checked)} />
        <Common.Form.Check.Label htmlFor="form_check">I agree to the terms and conditions</Common.Form.Check.Label>
      </Common.Form.Check.Wrapper>

      <FormNavigator responsive conditions={() => false} placeholder="Next" loading={loading} hasErrors={hasErrors} onSubmit={onSubmit} />
