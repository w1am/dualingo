...
const MerchantRegister = ({ onInputChange, onFormSubmit, clearInputField, hasErrors, formData, loading, setLoading, setTargetMessage }) => {
      ...
      findUser({
        ...
        if (!hasErrors && ok) {
          ...
          if (res.data.registerMerchant) {
            ...
            setSuccess(true);
            setTimeout(() => {
              location.assign(`/store/${formData['storeName'].value}/settings?new=true`);
            }, 1000)
          }
        } else {
          setLoading(false);
  ...
  return (
    <div>
      ...
      <FormNavigator conditions={() => false} placeholder="Register" loading={loading} hasErrors={hasErrors} onSubmit={onSubmit} />
    </div>
  )
}
