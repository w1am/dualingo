...
const headers = [ 'Name & Surname', 'Email Address', 'Mobile Phone', 'Password', 'Confirm Password' ];

const { Common, Forms, Animations, User: Usr } = Styles;
...
const UserRegister = ({ onInputChange, onFormSubmit, clearInputField, hasErrors, formData, loading, setLoading }) => {
  ...
  const onSubmit = async () => {
    ...
    if (!hasErrors) {
      ...
      if (ok) {
        ...
        window.scrollTo(0, 0);
        location.assign('/merchant/auth/register/success');
      } else {
  ...
  return (
    <div>
      ...
      <Common.Description style={{ marginTop: 0 }}>
        By clicking Register, you agree to our Terms and that you have read our Data use Policy, including Cookie use
      </Common.Description>

      <Common.Buttons.Auth responsive disabled={hasErrors || loading} onClick={onSubmit}>
        <ButtonLoader color="primary" placeholder="Register" loading={loading} />
      </Common.Buttons.Auth>

      <Common.Links.Small to='#' onClick={() => location.assign('/user/auth/login')} style={{ marginTop: 15 }}>
